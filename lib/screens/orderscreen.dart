...
class _OrderScreenState extends State<OrderScreen> {
  ...
  Future<void> _loadSessions() async {
    setState(() { _loadingSessions = true; _error = null; });
  ...
  Future<void> _selectSession(SessionModel s) async {
    setState(() { _selected = s; _loadingDetail = true; _detail = null; });
    final res = await ApiService.getSessionDetail(s.id);
    if (!mounted) return;
    setState(() { _detail = res.data; _loadingDetail = false; });
  }
    ...
    child: Row(children: [
          ...
          Row(children: [
            Container(width: 7, height: 7, margin: const EdgeInsets.only(right: 5),
                decoration: BoxDecoration(
                    color: _selected!.isOpen ? const Color(0xFFF59E0B) : _green,
      ...
      if (_selected != null)
        Container(
          ...
          decoration: BoxDecoration(
              color: _selected!.isOpen ? const Color(0xFFFEF3C7) : const Color(0xFFF0FDF4),
              borderRadius: BorderRadius.circular(8)),
          child: Text(_selected!.isOpen ? 'IN PROGRESS' : 'BILLED',
              style: TextStyle(
                  color: _selected!.isOpen ? const Color(0xFFD97706) : _green,
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: const Row(children: [
              ...
              SizedBox(width: 4),
              Text('New Session', style: TextStyle(color: Colors.white,
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Container(width: 32, height: 32,
                decoration: BoxDecoration(
                    color: isSelected ? _blue.withOpacity(0.15) : const Color(0xFFF3F4F6),
                    borderRadius: BorderRadius.circular(8)),
      ...
      child: Row(children: [
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          ...
          Text('\$${s.totalAmount.toStringAsFixed(2)}',
              style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: _blue)),
        ]),
      ]),
    );
  }
        ...
        if (items.isEmpty)
          Container(
            ...
            decoration: BoxDecoration(color: Colors.white,
                ...
                border: Border.all(color: const Color(0xFFE5E7EB))),
            child: const Center(child: Text('No items yet. Add some!',
  ...
  void _showStatusPicker(OrderItem item) {
    ...
    final available   = allStatuses.where((s) => allStatuses.indexOf(s) > currentIdx).toList();
            ...
            return GestureDetector(
              ...
              child: Container(
                ...
                child: Row(children: [
                  ...
                  const Spacer(),
                  Icon(Icons.arrow_forward_ios_rounded, color: colors[0], size: 14),
                ]),
              ),
            );
          }),
  ...
  List<Color> _statusColor(String s) {
    if (s == 'PREPARING') return [const Color(0xFFD97706), const Color(0xFFFEF3C7)];
    if (s == 'READY')     return [const Color(0xFF16A34A), const Color(0xFFF0FDF4)];
    if (s == 'SERVED')    return [const Color(0xFF2563EB), const Color(0xFFEFF6FF)];
    if (s == 'CANCELLED') return [const Color(0xFFDC2626), const Color(0xFFFEF2F2)];
  ...
  Widget _buildItemRow(OrderItem item) {
    Color fg, bg;
    if (item.status == 'READY')          { fg = _green;                     bg = const Color(0xFFF0FDF4); }
    else if (item.status == 'PREPARING') { fg = const Color(0xFFD97706);    bg = const Color(0xFFFEF3C7); }
    else if (item.status == 'SERVED')    { fg = _grey;                      bg = const Color(0xFFF3F4F6); }
    else if (item.status == 'CANCELLED') { fg = const Color(0xFFDC2626);    bg = const Color(0xFFFEF2F2); }
    else                                 { fg = const Color(0xFF7C3AED);    bg = const Color(0xFFF5F3FF); }
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (item.notes.isNotEmpty)
              Text(item.notes, style: const TextStyle(fontSize: 11, color: _grey)),
            if (item.unitPrice > 0)
  ...
  Widget _buildNoItems() => Center(child: Column(
    ...
    child: GestureDetector(
      onTap: () => Navigator.push(context, MaterialPageRoute(
          builder: (_) => AddItemsScreen(
              tableName: widget.table.name, bookingId: _selected!.id)))
...
class _NewSessionDialog extends StatefulWidget {
  ...
  @override State<_NewSessionDialog> createState() => _NewSessionDialogState();
}
